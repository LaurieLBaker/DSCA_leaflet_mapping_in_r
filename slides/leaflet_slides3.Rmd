...
subtitle: "Summer Institute in Computational Social Science, Covenant University, Nigeria 2022"
...
library(rgeoboundaries) ## Geographic administrative boundaries
...
--
`r icons::icon_style(icons::fontawesome("lightbulb", style = "solid"), scale = 1)` What are some other examples of point, line, and polygon data?
...
`r icons::icon_style(icons::fontawesome("info-circle", style = "solid"), scale = 1)` We can access administrative data using the `geoboundaries` function from the `rgeoboundaries` package.

]

.right-plot[

```{r first-country-1, ref.label='first-country-1', echo=FALSE, out.width="95%", fig.alt="Map of Nigeria: admin level 0"}
```
...
```{r first-country-2, ref.label='first-country-2', echo=FALSE, out.width="95%", fig.alt="Map of Nigeria and Chad: admin level 0."}
```
...
.right-plot[
```{r first-country-3, ref.label='first-country-3', echo=FALSE, out.width="95%", fig.alt="Map of Nigeria: admin level 1."}
```
]

---

```{r first-country-4, echo=FALSE}
nigeria %>%
  kableExtra::kable()
```
...
```{r nigeria-pop, eval=TRUE, echo = FALSE}
...
```{r nigeria-pop-spatial-join-2, eval=TRUE, echo = TRUE}
...
```{r nigeria-pop-spatial-join, eval=TRUE, echo = TRUE}
...
```{r first-nigeria-map3, eval=FALSE}
...
.right-plot[
```{r first-nigeria-map3, ref.label='first-nigeria-map3', echo=FALSE, out.width="95%", fig.alt="Map of Nigeria with admin 1 areas filled by population"}
```
]
...
```{r nigeria-true-size, fig.alt = "Map of the world showing Nigeria overlaid on Greenland. The map demonstrates how coordinate reference systems skew our perception of how big a country is", echo=FALSE, out.width = "50%"}

knitr::include_graphics(path = "images/Nigeria_true_size.PNG")
...
* We can access administrative boundaries for several countries using the `rgeoboundaries` package.
--
...
```{r first-leaflet-map-nigeria, eval=FALSE}
leaflet(data = nc) %>%
  addTiles() %>%
  setView(lng = 8.13, #<< 
          lat = 6.88, #<<
...
```{r first-leaflet-map-nigeria, ref.label='first-leaflet-map-nigeria', echo=FALSE, out.width="100%", fig.alt="Leaflet map of Nigeria with open street map background"}
```
]
...
* **Your Turn**: Try creating a leaflet map with the nigeria data.
]
