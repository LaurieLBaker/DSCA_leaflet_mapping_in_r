...
```{r kenya-true-size, fig.alt = "Map of the world showing Kenya overlaid on Greenland. The map demonstrates how coordinate reference systems skew our perception of how big a country is", echo=FALSE, fig.cap = "The True Size of (thetruesize.com)"}

knitr::include_graphics(path = "images/Kenya_true_size.png")

```

[The True Size Of at thetruesize.com](https://thetruesize.com/#?borders=1~!MTczMDY3NDE.MTE4OTczMjc*MjU3NDY1NzA(MzkyMTU3MA~!KE*OTYxNTIxNw.MTY3MDYyNzg)NA)
...
.right-plot[
```{r first-leaflet-map-kenya, ref.label='first-leaflet-map-kenya', echo=FALSE, out.width="100%"}
```
]
...
```{r first-leaflet-map1d-provider, ref.label='first-leaflet-map1d-provider', echo=FALSE, out.width="100%"}
```
...
.right-plot[
```{r first-leaflet-map1e, ref.label='first-leaflet-map1e', echo=FALSE}
```
]
...
```{r Color-brewer, echo = FALSE, out.width=550, fig.align='center', fig.cap="Map of U.S. Counties showing green blue colour scheme"}
...
* **Your Turn** Using `names(nc)`, change `nc$births1974` to the column which corresponds to number of births in 1979.
...
```{r leaflet-map-work, ref.label='leaflet-map-work', out.width="100%", echo=FALSE}
```
...
```{r work-leaflet-map2, ref.label='work-leaflet-map2', echo=FALSE, out.width="100%"}
```
...
```{r work-map-labels, ref.label='work-map-labels', echo=FALSE, out.width="100%"}
```
